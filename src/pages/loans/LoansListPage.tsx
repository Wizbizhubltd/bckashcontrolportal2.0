...
export function LoansListPage() {
  ...
  const [status, setStatus] = useState((searchParams.get('status') as LoanStatus | null) ?? '');
  ...
  const load = async (pageToLoad: number) => {
    ...
    try {
      const result = await loansApi.list({
        ...
        status: (status || undefined) as LoanStatus | undefined,
        page: pageToLoad,
        pageSize: PAGE_SIZE,
      });
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">Every loan across the network, by account number.</p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-left text-gray-500">
            <tr>
              ...
              <th className="px-4 py-3 font-medium">Applied Amount</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <tr>
                <td colSpan={5} className="px-4 py-6 text-center text-gray-400">No loans found.</td>
              </tr>
            ) : (
              items.map((item) => (
