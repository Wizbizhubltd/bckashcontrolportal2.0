...
export function ReusableInputField({
  ...
  return (
    <div className="space-y-1">
      ...
      {as === 'textarea' && (
        <textarea id={name} name={name} value={value} onChange={onChange} onBlur={onBlur} placeholder={placeholder} rows={rows} className={baseClassName} />
      )}

      {as === 'select' && (
        <select id={name} name={name} value={value} onChange={onChange} onBlur={onBlur} className={`${baseClassName} bg-white`}>
