    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            <button onClick={handleClose} className="absolute top-4 right-4 p-1 text-gray-400 hover:text-gray-600 rounded-lg hover:bg-gray-100 transition-colors">
              <XIcon size={18} />
            </button>
            ...
            <div className="flex items-start gap-4 mb-4">
              ...
              <div>
                ...
                <p className="text-sm font-body text-gray-500 mt-1">{description}</p>
              </div>
            </div>

            {inputType === 'textarea' && (
              <div className="mb-4">
                {inputLabel && <label className="block text-xs font-body font-medium text-gray-600 mb-1.5">{inputLabel}</label>}
                <textarea
                  value={inputValue}
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={handleClose} className="px-4 py-2 text-sm font-heading font-bold text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                Cancel
              </button>
              <button
            ...
            {confirmDisabled && disabledReason && <p className="text-xs text-amber-600 text-right mt-2">{disabledReason}</p>}
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>
